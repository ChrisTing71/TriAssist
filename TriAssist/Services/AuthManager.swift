...
//  TriAssist
//
//  Sign in with Apple: requires adding "Sign in with Apple" capability in
//  Xcode → Target → Signing & Capabilities → +.
//
//  Sign in with Google: requires a Google Cloud OAuth 2.0 iOS client ID.
//  Set it in Settings → Google Client ID field.
//
...
class AuthManager: NSObject {
    ...
    func signInWithGoogle() async {
        ...
        do {
            ...
            // Google iOS OAuth scheme = reversed client ID
            ...
            // 3. Open browser & wait for redirect
